...
export default function Board() {
  ...
  const fetchProject = async () => {
    try {
      ...
      setProject(found);
      if (found && found.owner && found.owner._id === user?.id) {
        setIsOwner(true);
      }
    } catch (err) {
      console.log(err);
  ...
  const handleAddMember = async (e) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/auth/finduser?email=${memberEmail}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const columnGradients = {
    'Todo': 'from-indigo-500/20 to-indigo-600/10',
    'In Progress': 'from-amber-500/20 to-amber-600/10',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <div className="max-w-7xl mx-auto p-6 lg:p-8">
        ...
        <div className="mb-8">
          <button
            ...
            className="group text-gray-500 hover:text-indigo-600 mb-4 flex items-center gap-2 transition-all duration-300 font-medium"
          >
            ...
            {isOwner && (
              <div className="flex gap-3">
                <button
                  ...
                  className="group relative bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-xl shadow-md flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="group relative bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-xl shadow-md flex items-center gap-2"
                >
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-indigo-500 rounded-xl p-4 mb-6 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <p className="text-indigo-800 text-sm">
                You are a <strong>Team Member</strong> — You can drag tasks and add comments. Only the owner can create tasks and add members.
              </p>
            </div>
          </div>
        )}

        {/* Kanban Board - Auto-height columns */}
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {COLUMNS.map((col) => (
              <div
                key={col}
                className={`bg-white rounded-2xl shadow-lg border-t-4 ${columnColors[col]} overflow-hidden transition-all duration-300 hover:shadow-xl`}
              >
                <div className={`bg-gradient-to-r ${columnGradients[col]} p-4 border-b border-gray-100`}>
                  <h3 className="text-gray-800 font-bold text-lg flex items-center justify-between">
                    <span className="flex items-center gap-2">
                      <span className="text-2xl">{columnIcons[col]}</span>
                      {col}
                    </span>
                    <span className="bg-white/80 text-gray-700 text-sm px-2.5 py-1 rounded-full font-semibold shadow-sm">
                  ...
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`p-4 space-y-3 transition-all duration-200 ${
                        snapshot.isDraggingOver ? 'bg-gradient-to-br from-gray-50 to-indigo-50' : ''
                      }`}
                      style={{
                        minHeight: '300px', // Normal base height
                        height: 'auto', // Automatically expands when content is more
                      }}
                    >
                          ...
                          {(provided, snapshot) => (
                            ...
                            >
                              <div className="flex items-start justify-between mb-2">
                                <h4 className="text-gray-800 font-semibold text-base group-hover:text-indigo-600 transition-colors line-clamp-2">
                                  {task.title}
                                </h4>
      ...
      {/* Create Task Modal - Professional Design */}
      ...
      {/* Add Member Modal - Enhanced Professional */}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl transform transition-all duration-300 animate-slideUp max-h-[90vh] overflow-y-auto">
            <div className="p-7">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  onClick={() => { setShowMemberModal(false); setMemberMsg(''); }}
                  className="text-gray-400 hover:text-gray-600 transition-colors p-1 hover:bg-gray-100 rounded-lg"
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              ...
              {memberMsg && (
                <div className={`mb-5 p-3 rounded-xl flex items-center gap-2 ${
                  ...
                  <p className={`text-sm ${memberMsg.includes('✅') ? 'text-green-700' : 'text-red-700'}`}>
                    {memberMsg}
                  </p>
                </div>
              )}
              ...
              <form onSubmit={handleAddMember} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 rounded-xl bg-gray-50 text-gray-800 border border-gray-200 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-200 transition-all"
                    value={memberEmail}
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="submit"
                    className="flex-1 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold py-3 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-md"
                  >
                    Add Member
                  </button>
                  <button
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {project?.members?.map((member) => (
                    <div key={member._id} className="bg-gray-50 p-3 rounded-xl flex items-center gap-3 hover:bg-gray-100 transition-all duration-200">
                      ...
                      {project.owner?._id === member._id && (
                        <span className="text-amber-600 text-xs font-semibold bg-amber-50 px-2 py-1 rounded-full">Owner</span>
                      )}
                    </div>
                  ))}
                </div>
